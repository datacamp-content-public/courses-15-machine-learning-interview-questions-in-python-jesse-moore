...
`@script`
We are going to try to predict the concentration of PM 2.5. 

To build our features, we first need to make sure that all data is sorted according to the timestamps. Then we will make use of the Pandas "shift" function. 
...
`@script`
We can see that the first rows value for PM10 has been appended to the second row. This occurs for every row in the dataset.
...
I want to use data from three previous time steps to predict the next time step.
...
Finally, we drop the NA values produced from shifting the data as well as the "PM10" value for each timestep as we will only be predicting PM2.5.
...
Remember how we used Pandas shuffle function. Since we randomly sampled the data, the algorithm will certainly have access to future information when making predictions.
...
`@script`
Our data is already ordered and ready to go.

The first step is to find the index that covers the training part of our data. We will use the first 75% of our dataset.
...
Finally, we drop the 'date' column from our prepared data before making predictions.
...
`@part2`
![](https://i.imgur.com/31eQGJ8.png){{1}}
