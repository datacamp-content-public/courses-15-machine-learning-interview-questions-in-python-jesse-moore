...
We have available air_df which contains data for one sensor in Madrid during 2017. 
...
Remember how we used the shift function. Since we randomly sampled the data, the algorithm will have access to future information.

In this case, your algorithm could determine from some of the features in the dataset to "Look Ahead".

The result, and algorithm that tests well but performs poorly.
...
Second, we get the integer index that will mark the training/test split of the dataset.

Third, we create our training and validation dataframes using the split index we just found.
